...
const App = () => {
    ...
    // -- Refactored State for Chat History --
    const [chats, setChats] = useState<{ [chatId: string]: Chat }>({});
    ...
    useEffect(() => {
        if (selectedClass) {
            ...
            // If no chats exist for this class, create one.
            if (!chatHistories[selectedClass] || Object.keys(chatHistories[selectedClass]).length === 0) {
                ...
                // If no active chat is set, set it to the first one.
    ...
    useEffect(() => {
        if (activeChatId && !chats[activeChatId]) {
            initializeChat(selectedClass as number, activeChatId);
        }
    }, [activeChatId, selectedClass]);
    ...
    // === Core Logic ===
    const initializeChat = (classNum: number, chatId: string) => {
        const systemInstruction = `You are bhsAI, a friendly, academic, and highly creative AI assistant for a Class ${classNum} student of Birla High School Mukundapur. Your responses must be encouraging, easy to understand, and strictly tailored to the NCERT syllabus for Class ${classNum}. You must decline to answer any questions that are not related to academics, are inappropriate, or are unrelated to the student's curriculum. Prioritize safety and relevance in all interactions.`;
    ...
    const handleSendMessage = async (messageText: string) => {
        ...
        setChatHistories(prev => ({
            ...
            [selectedClass]: {
                ...prev[selectedClass],
                [activeChatId]: [...(prev[selectedClass]?.[activeChatId] || []), userMessage, { role: 'model', text: '' }]
            }
        }));
        ...
        try {
            if (isGoogleSearchEnabled && !imageFile) {
                ...
                updateLastMessage({ role: 'model', text: response.text, sources });
            } else {
                const chatInstance = chats[activeChatId];
                if (!chatInstance) throw new Error("Chat not initialized");

                const stream = await (imageFile 
                    ? ai.models.generateContentStream({
                        model: 'gemini-2.5-flash',
                        contents: { parts: [await fileToGenerativePart(imageFile), { text: messageText }] }
                    })
                    : chatInstance.sendMessageStream({ message: messageText })
                );
            ...
            updateLastMessage({ role: 'model', text: 'Sorry, something went wrong. Please try again.' });
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleNewChat = (classNum?: number) => {
        ...
        initializeChat(targetClass, newChatId);
    };
            ...
            <style>{`
                ...
                * { box-sizing: border-box; margin: 0; padding: 0; }
                ...
                @keyframes title-glow {
                    0%, 100% {
                        text-shadow: 0 0 8px rgba(249, 119, 33, 0.4), 0 0 16px rgba(249, 119, 33, 0.4);
                    }
                    50% {
                        text-shadow: 0 0 12px rgba(45, 121, 199, 0.6), 0 0 24px rgba(45, 121, 199, 0.6);
                    }
                }
                ...
                .title-main .gemini-gradient-text { 
                    font-weight: 700; 
                    animation: title-glow 3s ease-in-out 1;
                }
                ...
                .class-button:hover, .sidebar-btn:hover {
                    ...
                    box-shadow: 0 0 20px rgba(136, 215, 228, 0.6);
                }
